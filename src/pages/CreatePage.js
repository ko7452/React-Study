...
const CreatePage = () => {
  ...
  const getValue = e => {
    ...
    // blogContent의 내용을 복사해서 그 안에 name이라는 이름의 키의 값을 value로 바꿔 저장한다는 의미
    setBlogContent({
      ...blogContent,
      [name]: value,
    });
  ...
  // 내용을 보면, 버튼을 눌렀을 때 viewContent라는 배열 안에 movieContent라는 객체를 복사해서 concat 해준다음 그 내용으로 변경 해주는 내용
  const buttonOnClickEvent = () => {
    setViewContent(viewContent.concat({ ...blogContent }));
  };
